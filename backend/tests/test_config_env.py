...
def test_env_file_is_actually_loaded(tmp_path, monkeypatch) -> None:
    ...
    created = []
    try:
        env_file.write_text("KINTSUGI_CODE_LOG_LEVEL=FROM_BACKEND_ENV\n", encoding="utf-8")
        created.append(env_file)
        if not stray.exists():
            stray.parent.mkdir(parents=True, exist_ok=True)
            stray.write_text("KINTSUGI_CODE_LOG_LEVEL=FROM_APP_STRAY\n", encoding="utf-8")
            created.append(stray)
        monkeypatch.delenv("KINTSUGI_CODE_LOG_LEVEL", raising=False)
        s = Settings()
...
def test_env_vars_take_precedence_over_env_file(tmp_path, monkeypatch) -> None:
    ...
    try:
        if not env_file.exists():
            env_file.write_text("KINTSUGI_CODE_LOG_LEVEL=FROM_FILE\n", encoding="utf-8")
            wrote = True
        monkeypatch.setenv("KINTSUGI_CODE_LOG_LEVEL", "FROM_PROCESS_ENV")
        s = Settings()
